...
namespace BetterCallender
{
    public partial class MainPage : ContentPage
    {
        ...
        public class AutoGeneratedGrid : Grid
        {
            ...
            public AutoGeneratedGrid(DateTime date, MainPageViewModel viewModel)
            {
                ...
                foreach (var dayViewModel in ViewModel.Days)
                {
                    ...
                    var column = (day - 1 + startingDayOfWeek) % 7;
                    ...
                    dayLabel.SetValue(Grid.RowProperty, (day - 1 + startingDayOfWeek) / 7 + 1);
                    dayLabel.SetValue(Grid.ColumnProperty, column);
        ...
        private void GenerateInitialCalendarGrids(int year, int selectedMonth)
        {
            stackLayout = new StackLayout(); // Initialize the private field

            Content = stackLayout;

            var initialDate = new DateTime(year, selectedMonth, 1);
            GenerateCalendarGrid(initialDate);
        ...
        public void ChangeDisplayMode()
        {
            if (isScrollable)
            {
                ...
                var buttonGrid = CreateLayout();
                stackLayout.Children.Add(buttonGrid);
                int currentYear = int.Parse(ViewModel.CurrentYear);
                GenerateInitialCalendarGrids(currentYear, ViewModel.SelectedMonth);
            }
            else
            {
                isScrollable = true;
        ...
        {
            var selectedDate = new DateTime(Convert.ToInt32(ViewModel.CurrentYear), ViewModel.SelectedMonth, 1);
            var initialDate = selectedDate;

            stackLayout.Children.Clear();

            scrollView = new ScrollView(); // Přiřazení ke třídě
            stackLayout.Children.Add(scrollView);

            var innerStackLayout = new StackLayout();
            scrollView.Content = innerStackLayout;

            for (int i = 0; i < 3; i++)
            {
                var date = initialDate.AddMonths(i);
                GenerateCalendarGrid(date, i == 0);
            }

            stackLayout.Children.Add(masterStackLayout);
            stackLayout.Children.Add(labelScrollView);
        }
    }
}